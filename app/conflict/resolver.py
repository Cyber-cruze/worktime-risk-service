...
def resolve_conflict(request: ConflictResolveRequest) -> ResolutionResponse:
    ...
    if conflict.type == ConflictType.OUTSIDE_WORK_HOURS:
        ...
        recommendations.append(Recommendation(
            ...
            reason=reason, confidence=0.85, affected_user_ids=[conflict.userId]
        ))
        ...
        recommendations.append(Recommendation(
            ...
            reason="Найдено свободное окно через 1 час после текущего", confidence=0.7,
        ...
        recommendations.append(Recommendation(
            action="DELEGATE", reason="Передать задачу коллеге со схожей специализацией или меньшей загрузкой",
            confidence=0.75, affected_user_ids=[conflict.userId]
        ))
        recommendations.append(Recommendation(
            action="RESCHEDULE", reason="Отложить задачу на следующий спринт или неделю",
            confidence=0.6, affected_user_ids=[conflict.userId]
        ))
        ...
        recommendations.append(Recommendation(
            action="KEEP", reason="Проверить статус исключения в HR-системе. Если подтверждено — оставить.",
            confidence=0.5, affected_user_ids=[conflict.userId]
        ))
        recommendations.append(Recommendation(
            action="CANCEL", reason="Отменить событие, если исключение не подтверждено HR",
            confidence=0.4, affected_user_ids=[conflict.userId]
        ))
        status = "MANUAL_REVIEW"
    ...
    if use_llm and recommendations:
        try:
            ...
            llm_text = generate_conflict_explanation(
                ...
                new_time=time_str,
                all_actions=[r.action for r in recommendations]
            )
